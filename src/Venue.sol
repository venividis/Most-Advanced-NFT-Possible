...
contract Venue {
    ...
    ///         then offers v3 only and says so. v4's `initialize` is the one
    ///         v4 entry point a client with no ABI coder can reach on its
    ///         own: `PoolKey` is five static fields, so
