...
contract DeskLaunch {
    ...
    string internal constant LAUNCH_JS =
        ...
        /*  launch(string,string,uint8,uint256,bytes32)
            head is five words: two offsets then three flat values.       */
        "const launchData=(n,y,d,v,salt)=>{"
        "const o1=5*32,o2=o1+32*(1+(n.length?Math.ceil(n.length/32):0));"
        "return S.launch+I.W(o1)+I.W(o2)+I.W(d)+I.W(v)+I.pad(salt)"
        "+TAIL(n)+TAIL(y)};"
        ...
        "return{n:n,y:y,d:d,v:v,salt:salt}};"
        ...
        "$('cpre').innerHTML='<div><span>it would land at</span><b>0x'"
        "+String(r).slice(26,66)+'</b></div>'"
        "+'<div><span>supply</span><b>'+I.fmt(f.v,f.d,6)+' '+f.y+'</b></div>';"
        "say('read from the launchpad \\u00b7 nothing sent','ok')});"

        "onc('cgo',async()=>{const f=form();"
        "await I.send(K.kiln,launchData(f.n,f.y,f.d,f.v,f.salt))});"
        ...
        "const mine_=$('cpre').textContent.match(/0x[0-9a-f]{40}/);"
        "const a=mine_?mine_[0]:null;"
