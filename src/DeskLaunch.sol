...
contract DeskLaunch {
    ...
    string internal constant LAUNCH_JS =
        ...
        "await I.send(t,S.approve+I.AD(K.permit2)+I.W(amt));"
        "await I.send(K.permit2,S.permitApprove+I.AD(t)+I.AD(K.positionManager)+I.W(amt)+I.W(f.deadline))}"
        "say('approval requests sent \\u00b7 rebuild the plan if anything changed','ok')});"
        "on('lgo',async()=>{await I.connect();if(!liqPlan)throw new Error('build and inspect the position first');"
        "await I.send(K.positionManager,liqPlan.data,liqPlan.value)});"
