...
web3://<premises>/open                  every token currently open for business
web3://<premises>/token/42              one token's counter
web3://<premises>/token/42/market       quote and trade against it
...
### The pages transact, and the browser computes almost nothing

A storefront that cannot transact is a brochure, and building a transaction
normally means shipping an ABI coder and a keccak-256 to the browser. Not here:
the page is generated by a contract, and Solidity has keccak. Every selector,
and every argument the page already knows, is encoded on chain and arrives as a
finished calldata prefix; the client appends the words the visitor typed. What
is left is about forty lines. A visitor can read `data-call` in the page source
and check it against the ABI themselves — the selector came out of a contract,
not out of the page's JavaScript.
...
true, then attack it. [INVARIANTS.md](INVARIANTS.md) lists seventy-four such
